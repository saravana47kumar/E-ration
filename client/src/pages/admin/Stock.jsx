  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      ...
      <main className="flex-1 p-6 lg:p-8">
        ...
        {tab === 'add' && (
          <div className="max-w-md"><div className="card">
            ...
            <form onSubmit={handleAddStock} className="space-y-4">
              ...
              <div><label className="label">Quantity to Add</label>
                <input type="number" className="input" value={addForm.quantity} onChange={e => setAddForm({ ...addForm, quantity: e.target.value })} required min={1} /></div>
        ...
        {tab === 'allocate' && (
          <div className="max-w-md"><div className="card">
            ...
            <form onSubmit={handleAllocate} className="space-y-4">
              <div><label className="label">Distributor</label>
              ...
              <div><label className="label">Quantity</label>
                <input type="number" className="input" value={allocForm.quantity} onChange={e => setAllocForm({ ...allocForm, quantity: e.target.value })} required min={1} /></div>
          ...
          <div className="card overflow-x-auto">
            <table className="w-full">
              <thead className="border-b border-gray-100"><tr>
                <th className="table-th">Product</th><th className="table-th">Distributor</th>
                <th className="table-th">Allocated</th><th className="table-th">Used</th><th className="table-th">Available</th>
              </tr></thead>
        ...
        {tab === 'requests' && (
          <div className="card overflow-x-auto">
            <table className="w-full">
              <thead className="border-b border-gray-100"><tr>
                <th className="table-th">Distributor</th><th className="table-th">Product</th>
                <th className="table-th">Requested</th><th className="table-th">Reason</th>
        ...
        <Modal isOpen={!!handleModal} onClose={() => setHandleModal(null)} title="Handle Stock Request">
          <form onSubmit={handleRequest} className="space-y-4">
            ...
            {handleForm.status === 'approved' && (
              <div><label className="label">Approved Quantity</label>
                <input type="number" className="input" value={handleForm.approvedQuantity} onChange={e => setHandleForm({ ...handleForm, approvedQuantity: e.target.value })} min={1} required /></div>
            )}
            <div><label className="label">Admin Note</label>
              <textarea className="input" rows={2} value={handleForm.adminNote} onChange={e => setHandleForm({ ...handleForm, adminNote: e.target.value })} /></div>
