...
const placeOrder = async (req, res) => {
  try {
    ...
    for (const item of items) {
      const productId = item.product || item.productId;
      await Product.findByIdAndUpdate(productId, { $inc: { availableStock: -item.quantity } });
    }

    // Send SMS for COD orders (non-blocking)
    if (paymentMethod === 'cod') {
      // Use setTimeout to make SMS non-blocking
      setTimeout(async () => {
        try {
          await sendOrderPlacedSMS(customer, order);
        } catch (smsError) {
